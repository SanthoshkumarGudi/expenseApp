...
import {
  ...
  InputLabel, Chip, Avatar, IconButton, Tooltip, Alert, Table,
  TableBody, TableCell, TableContainer, TableHead, TableRow,
  TableSortLabel, TablePagination, Paper, Stack, InputAdornment,
  Button, Skeleton, Collapse, Divider,
} from "@mui/material";
import {
  Search, Refresh, Clear, CheckCircle,
  Cancel, PersonOff, Edit, PersonAdd, TuneOutlined,
...
// show: which breakpoint the column starts appearing at
const COLUMNS = [
...
const RoleChip = ({ role }) => {
  const colorMap = { admin: "error", manager: "warning", hr: "info", employee: "default" };
  return (
    <Chip
      label={role ?? "—"} size="small"
      color={colorMap[role?.toLowerCase()] ?? "default"} variant="filled"
...
export default function EmployeeList() {
  ...
  return (
    <Box sx={{ p: { xs: 1.5, sm: 2, md: 3 }, display: "flex", flexDirection: "column", gap: { xs: 2, md: 3 } }}>
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 1.5 }}>
        ...
        <Stack direction="row" gap={1} alignItems="center">
          ...
          >
            ...
            <Box component="span" sx={{ display: { xs: "none", sm: "inline" } }}>Add employee</Box>
      ...
      <Paper variant="outlined" sx={{ borderRadius: 3, overflow: "hidden" }}>
        <Box sx={{ p: { xs: 1.5, sm: 2 }, display: "flex", gap: 1, alignItems: "center" }}>
          <TextField
            size="small"
          ...
          <Button
            ...
            sx={{ px: { xs: 1, sm: 1.5 }, py: 1, flexShrink: 0, whiteSpace: "nowrap" }}
          >
            <Box component="span" sx={{ display: { xs: "none", sm: "inline" } }}>
              Filters{activeFilterCount > 0 ? ` (${activeFilterCount})` : ""}
            </Box>
            {/* On xs just show count badge if any */}
            {activeFilterCount > 0 && (
          ...
          {hasActiveFilters && (
            <Tooltip title="Clear all filters">
              <IconButton size="small" onClick={clearAll} sx={{ flexShrink: 0 }}>
                <Clear fontSize="small" />
              </IconButton>
            </Tooltip>
          )}
        ...
        <Collapse in={filtersOpen}>
          ...
          >
            <FilterSelect id="grade"      label="Grade"      value={filters.grade}      onChange={(v) => handleFilterChange("grade", v)}      options={GRADE_OPTIONS}  />
            <FilterSelect id="department" label="Department" value={filters.department} onChange={(v) => handleFilterChange("department", v)} options={DEPT_OPTIONS}   />
            <FilterSelect id="role"       label="Role"       value={filters.role}       onChange={(v) => handleFilterChange("role", v)}       options={ROLE_OPTIONS}   />
            <FilterSelect id="status"     label="Status"     value={filters.status}     onChange={(v) => handleFilterChange("status", v)}     options={STATUS_OPTIONS} />
          </Box>
        </Collapse>
      </Paper>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={COLUMNS.length} align="center" sx={{ py: 8 }}>
                    ...
                    {hasActiveFilters && (
                      <Button size="small" onClick={clearAll} sx={{ mt: 1 }}>Clear filters</Button>
                    )}
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={emp.id} hover sx={{ "&:last-child td": { borderBottom: 0 } }}>
                    ...
                    <TableCell sx={{ px: { xs: 1, sm: 1.5, md: 2 } }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <Avatar sx={{ width: { xs: 28, sm: 32 }, height: { xs: 28, sm: 32 }, fontSize: "0.7rem", bgcolor: "primary.main", flexShrink: 0 }}>
                          {initials(emp.display_name)}
                        </Avatar>
                        <Box sx={{ minWidth: 0 }}>
                          <Typography variant="body2" fontWeight={600} noWrap sx={{ maxWidth: { xs: 90, sm: 140, md: 180 } }}>
                            {emp.display_name}
                          </Typography>
                          {/* Email under name only when email column isn't visible */}
                          <Typography variant="caption" color="text.secondary" noWrap
                            sx={{ display: { xs: "block", md: "none" }, maxWidth: { xs: 90, sm: 160 } }}>
                            {emp.work_email}
                          </Typography>
                        </Box>
                      </Box>
                    ...
                    {/* ID — sm+ */}
                    <TableCell sx={{ display: showDisplay("sm"), px: { xs: 1, sm: 1.5, md: 2 } }}>
                    ...
                    {/* Department — md+ */}
                    <TableCell sx={{ display: showDisplay("md"), px: { xs: 1, sm: 1.5, md: 2 } }}>
                      <Typography variant="body2">{emp.department ?? "—"}</Typography>
                    </TableCell>

                    {/* Designation — lg+ */}
                    <TableCell sx={{ display: showDisplay("lg"), px: { xs: 1, sm: 1.5, md: 2 } }}>
                      <Typography variant="body2" noWrap sx={{ maxWidth: 140 }}>{emp.designation ?? "—"}</Typography>
                    </TableCell>

                    {/* Grade — md+ */}
                    <TableCell sx={{ display: showDisplay("md"), px: { xs: 1, sm: 1.5, md: 2 } }}>
                      <Chip label={emp.grade ?? "—"} size="small" variant="outlined" sx={{ fontWeight: 700, minWidth: 32 }} />
                    </TableCell>
                    ...
                    {/* Joined — md+ */}
                    <TableCell sx={{ display: showDisplay("md"), px: { xs: 1, sm: 1.5, md: 2 } }}>
                      <Typography variant="body2" noWrap>{formatDate(emp.date_of_joining)}</Typography>
                    </TableCell>
